...
def _mint(new_mm, prior):
    summaries = [("S1", {"cwd": "/r/mw-cli", "last_activity_at": "2026-06-06T06:36:32Z"},
                  "", RAW_FM)]
    return classify.mint_sealed_initiatives(new_mm, summaries, [], prior)


def test_dropped_sealed_resurrects_with_prior_name():
    prior = {"workspaces": [{"name": "mw-cli", "initiatives": [PRIOR_CARD]}]}
    new_mm = {"workspaces": [{"name": "mw-cli", "cwd": "/r/mw-cli", "initiatives": []}]}
...
def test_existing_sealed_left_frozen():
    prior = {"workspaces": [{"name": "mw-cli", "initiatives": [PRIOR_CARD]}]}
    new_mm = {"workspaces": [{"name": "mw-cli", "cwd": "/r/mw-cli",
