...
4. 冒烟测试：`bash bin/stray --你的flag` 和 `bash bin/stray --help`
...
（`stray --refresh && tail Library/Logs/claude-stray.log`）。
...
- 升 `dashboard.json.schema_version` 需要 `render.py` 和 `render-html.py`
...
1. `python3 bin/render.py` 对当前 `cache/dashboard.json` 跑不报错
2. `bash bin/refresh.sh` 干净退出（force 或 hash-skip 都行）
3. `stray --diagnose` 对活跃 session 报全绿
4. `stray --serve` 起得来，能服务 `/`，能接受 `/api/save`，能
   Ctrl-C 干净退出
