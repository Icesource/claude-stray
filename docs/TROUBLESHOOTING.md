...
```bash
stray --diagnose [session_id]    # auto-picks most recent if omitted
```
...
## "Dashboard is empty" / "sync failed" (first run)

The cockpit now states the cause on the page itself. The usual ones:

- **`claude` not logged in** — the background analysis can't run.
  Check with `claude -p hi`; if it fails, log in, then `stray --refresh`.
- **Still syncing** — first run analyzes your recent sessions in the
  background (~1–2 min); the page shows a "first sync…" state.
- **Nothing in the last ~48h** — first run only syncs recent sessions.
  Pull in your full history with `stray --backfill`.
- **`claude` not installed / not on PATH** — the page says so; install
  Claude Code and re-open `http://127.0.0.1:9876/`.

## "My new task / fix / decision isn't showing in the dashboard"
...
| Stage 3 (mindmap) `session_id NOT in any initiative` AND last AI run < session activity | AI hasn't run since session was extracted | `stray --refresh` |
...
| Stage 5 `in cooldown` | A real AI run happened recently; the next hook-triggered run is gated | Wait, or `stray --refresh` to force |

### Step 3: force a real run

```bash
stray --refresh    # sets CLAUDE_WORKTREE_FORCE=1 inside refresh.sh
```
...
the dashboard.json mtime/content doesn't move.
...
Before commit `9f01447` the cooldown gate used `dashboard.json` mtime. The
...
dashboard.json BEFORE the AI sees PRIOR_MINDMAP, and the prompt has a
...
2. Wait for one hook-driven refresh, then check again — file should be
   cleared (consumed) and `cache/dashboard.json` should show the task done
...
removed from `dashboard.json` and the data lives in `cache/archive/<ws>/
...
2. Check `cache/dashboard.json` does NOT have that id under any workspace
3. If both are correct but the card still shows in a workspace, restart
   the server (HTML may be cached). `stray --serve` regenerates HTML
   on each GET when dashboard.json is newer than dashboard.html

## "`stray --serve` won't shut down with Ctrl-C"
